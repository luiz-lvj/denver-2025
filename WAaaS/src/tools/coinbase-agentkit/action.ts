...
// Define the schema for the Coinbase AgentKit tool
export const CoinbaseAgentKitSchema = z.object({
...
async function coinbaseAgentKitFunc(
  args: z.infer<typeof CoinbaseAgentKitSchema>
): Promise<string> {
  const { operation, assetPair, amount, side } = args;
...
// Export the Coinbase AgentKit action
export const coinbaseAgentKitAction: WaaaSAction<typeof CoinbaseAgentKitSchema> = {
  name: "coinbase_agentkit",
  description: "Interact with Coinbase for cryptocurrency operations",
  schema: CoinbaseAgentKitSchema,
  func: coinbaseAgentKitFunc
}; 